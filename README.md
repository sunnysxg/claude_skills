...
| `session-log` | 在 Cursor / Claude Code / Codex 归档并按 UUID upsert 同一 chat；Codex Desktop / Claude Code Desktop 在宿主工具可用时直接改当前 chat 标题（CCD 传 `"self"`，并顺手补历史遗留），Claude/Cursor CLI 可选 Stop hook 执行标题建议 |
...
| `taskboard` | 操作中央待办看板（dashi taskboard/todo_hub）：读卡、认领、改状态、评论，全部走 `taskctl` CLI；SKILL.md 只留每轮通用的规矩和一张按 prompt 原文判轮次的路由表，人手交付、交付评论、讨论卡交回、完成收尾、原理留档页、自动轮补充各在 `references/`，自动派发与预调研轮以派发须知为准 |
| `ui-options` | 用户要比较 UI 呈现 / 交互设计方案时，用项目自己的设计 token 在项目草稿目录 `_sxg/artifacts/<卡号小写>/` 生成一页本地 HTML 对比（几个方案 × 几种情形 × 宽度），侧栏渲染让她选，选定后回写看板；不用 Artifact |
