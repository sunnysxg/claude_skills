...
## 2026-09-15 原理留档页只对看板仓自己的卡触发

状态：现行

在「taskboard 纪律 6 规定同笔追加决策记录拍板节就做一页原理留档页，而留档页的形式是随候选同笔 commit 进 todo_hub 的 `dashi-taskboard/web/public/notes/`；别的项目的卡（候选树在 claude_skills 等仓）同样会追加自己仓的拍板节，候选里却没有这个目录」的场景下（CLAUDESKILLS-25、-28 各实撞一次，都只能在交付评论里说明跳过），面对「规则对一半的卡不可执行，每次都要 agent 临场解释为什么没做」的顾虑，选「触发条件收窄为看板仓（todo_hub）自己的卡；别的项目的卡追加拍板节不做留档页、不加『[Sarah] 看懂原理』项，也不必在交付评论里说明跳过」，否「留档页始终进看板仓、由 worker 另开一笔提交到 todo_hub」「留档页落本项目仓（如 `docs/notes/`），看板上只给文件路径」，以达「规则对每张会触发它的卡都可照做」，接受「别的项目的机制类拍板没有人话展开版，只有拍板节本身」。

否决理由：
- 另开一笔提交到 todo_hub：生产卡的发布 saga 只合并本卡候选分支，跨仓提交得由 worker 自己 commit/push，与「不自行 merge / push」纪律冲突，要专门开口子；自动派发会话还被禁止改别的仓。
- 落本项目仓只给路径：手机上打不开，而留档页的交付入口设计就是一条手机和电脑都能点的相对链接。
