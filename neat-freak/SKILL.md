...
- [ ] 本会话推进的卡已停在用户会看的状态（等验收 `in_review`、缺输入 `blocked`）；「等你处理」只剩必须用户动手的少数项。
...
1. **待办**：经 `taskboard` 回写中央看板（board 不可达时按
   ...
   改动、验证、遗留落进评论；状态挪到用户会看的位置——等验收挪 `in_review`，要用户输入
   才能继续挪 `blocked`，共同讨论进行中留 `in_progress`，不得停在 `backlog` 指望被看到。
