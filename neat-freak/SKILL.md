...
4. **补 AI 规则文件**：项目有可运行代码但没有任何规则文件时，默认创建一份最小规则文件（按当前平台的原生名字：Claude Code 用 CLAUDE.md，其他多数平台用 AGENTS.md），只写五件事：项目一句话定位、怎么跑起来、技术栈、目录与约定、当前状态和下一步。控制在 60 行内——这份文件是下次会话恢复上下文的入口，不是第二份 README。已有规则文件则只修矛盾和过期项，不推倒重写；改了就按「知识放在哪里」的常驻预算量一次。
...
| 位置 | 只保留什么 |
|---|---|
| CLAUDE.md / AGENTS.md / rules | 下次 Agent 不看到就会犯错的边界、命令和工作流，以及「改哪块先读哪份 docs」的路由；受常驻预算约束（见表下）；不放拍板日期、否决过的方案、交付流水，也不放系统怎么工作、某个判据住在哪个文件、踩过的坑 |
| README / docs（或该源码文件的文件头） | 系统如何使用、工作、运维，当前外部合同；某个判据住在哪个文件、踩过的坑——按需读，不常驻 |
...
**常驻预算**：规则文件每个会话都整份加载。一条常驻链 = 入口规则文件（CLAUDE.md / AGENTS.md）连同它 `@import` 的文件和不带 `paths:` 的 rules，全局链与各项目链分开算，目标 200 行 / 24,000 字符以内。本次收口改了规则文件，就量它所在的那条链（量字符不量字节，中文一字 3 字节）：

```bash
LC_ALL=C.UTF-8 wc -lm <入口规则文件> <它导入的文件>...   # total 行 = 行数、字符数
```

- 超预算：本次新增的内容写进 docs（或该源码文件的文件头），规则文件里只加一行路由；不往规则文件里追加叙述。存量超出的部分不在收口里整体搬迁，汇报里点出超了多少。
- 汇报里给出读数：`<入口>：N 行 / M 字符，未超` 或 `…，超预算`。没改规则文件不量。
...
- 规则层只保留可复用约束；机制进 docs，历史进 git/changelog/事故文档。改了规则文件就量常驻预算，超了新增内容先进 docs、规则只留路由（见「知识放在哪里」）。
...
3. **等你处理**：越权、破坏性或无法裁决的项，加上本会话等验收的卡和必须用户拍板的决定。
   先自消化——能自决的自决、能延后的建卡延后，剩下的逐项一行，必须很少。每项自包含：
   点名项目与卡号（或文件/对象）+ 要用户做的动作，用户只看这一条就能明白，不用回溯上下文
   ——只说「验收本次改动」而不点名哪张卡，等于没说。只列本会话的事，看板自己会显示的
   全局状态不重复。
...
**等你处理**
- <项目 + 卡号/对象 + 要你做的动作，逐项一行、每项自包含；没有就写「无」>

**常驻预算**：<改过的规则链：入口 + N 行 / M 字符 + 未超或超预算；没改规则文件就删掉这一行>

**遗留**：<pending / out-of-scope / 未消除 warning；没有就写「无」>
```

必须明确列出 `pending`、`out-of-scope` 和未消除的 warning，并在存在未获准的待清场现场时写明「复核现场仍保留，等待用户确认后清场」；不能用「保证干净」掩盖它们。确认后只补充汇报实际删除项、清场审计和残留 warning，不重写第一阶段的完整结果。已有精确授权并已清理时，在首次完整汇报中给出相同证据。常驻预算读数按「知识放在哪里」报告；其他体量读数只在现场规则定义了门槛时报告。
...
- [ ] 文档和规则没有新增流水账；改过的规则链已量常驻预算、读数进了汇报，超预算时新增内容进了 docs、规则文件只加了路由。
...
- [ ] 本会话推进的卡已停在用户会看的状态（等验收、等用户拍板都是 `in_review`；`blocked` 只留等下去有代价且系统自愈已用尽的；看板自动起的轮次由派发器挪，不自己挪）；「等你处理」只剩必须用户动手的少数项。
- [ ] 没有把宽泛「做完后清理」当成任意删除授权，也没有对已经精确授权的目标机械要求二次确认。
- [ ] 仅清理授权覆盖的精确目标；最终工作区重新审计，实际删除项、残留和 warning 已如实报告。

## 本仓库的本地收尾链

neat-freak 是收尾时用户唯一要主动运行的入口——一个会话何时收尾只有用户能判断，收尾不设
自动触发；任何「收尾时希望运行」的新任务挂进本链的固定顺序，不自立用户入口。执行情况靠
`session-log` 的改名在侧栏可见：未改名 ≈ 无归档工作（纯问答会话不改名是合法情形，不是
漏收尾）。为什么这么定、否决过什么见本仓 `docs/decisions.md`。

在其他事实面对齐后，按固定顺序执行三个本地入口；没有实际内容时不要为了形式写入：

1. **待办**：经 `taskboard` 回写中央看板（board 不可达时按
   `~/Projects/todo_hub/todo_convention.md` 写 `_sxg/inbox/`）。逐张核对本次认领或推进的卡：
   改动、验证、遗留落进评论；状态挪到用户会看的位置——等验收、要用户拍板才能继续，两类都挪
   `in_review`（`blocked` 只留等下去有代价且系统自愈已用尽的），共同讨论进行中留
   `in_progress`，不得停在 `backlog` 指望被看到。看板自动起的轮次（指令里有「自动派发会话须知」
   「自动预调研会话须知」）本卡状态由派发器挪，自己只写评论，判据见 taskboard「先认这一轮：读哪份」。
   再想一遍本次有没有该新建的卡，有就建。本次完成对旧卡的影响（被顺带解决、blocker 已清却
   停在 `backlog`）按 taskboard 纪律在每张卡做完时已查，多任务会话在此补核一遍有无漏网。
   尚未接入看板的机器上、项目仍维持旧
   `_sxg/TODO.md` 的，就地更新它：已完成项删除，稳定机制移入权威 docs/rules，不写成
   第二份架构文档。无论哪种，文档都不镜像卡片状态——待办的事实源只有一个。
