...
| 发布流程 | “清场目标必须精确且有明确授权” | knowledge receipt、deploy marker、cleanup candidate、user authorization receipt |
...
用户已明确指定目标和动作时，不需要为了形式重复确认。执行前仍要解析最终路径、核对作用域、
检查链接/唯一改动和恢复边界；如果目标有歧义、授权只覆盖部分对象，或检查发现影响面扩大，
把新增部分列为待决。执行后报告实际改动以及是否可恢复。
...
- **过载**：核心规则被历史叙事挤没，或常驻链超预算（口径见 SKILL.md「知识放在哪里」）；优先删/迁，而不是继续加目录。
