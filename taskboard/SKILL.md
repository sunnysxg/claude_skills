...
6. 做完并自验后：评论写清改动、验证方式、结果、遗留风险。凡要挪 `in_review` 给用户验收，
   验证方式必须是一段可直接照做的验收脚本：写明入口、已准备的初始状态/测试数据、顺序操作、
   各关键步骤的预期结果；核心结果若藏在 Git、文件或数据库等界面外，再给具体路径/命令并说明
   它与页面对象的对应关系。只给 URL、路径、测试命令或“测试通过”不算验收说明；一次性夹具
   交付时保持未消费，已经消费就重建或明确只能验结果。再按可验性分流（拍板见
...
7. 验收分流自通过之外，`done` 只在用户明确验收或明确要求完成后设，不从 `in_progress`
   ...
   **自动派发的会话**（prompt 含「自动派发会话须知」或「收尾会话」）例外：卡已由派发器认领并
   绑定，不再认领；只在给定的树里干活、commit 到当前分支、评论；不合并、不 push、不挪
   `in_review` / `done`——会话退出后由派发器 squash 合进该仓的 `auto` 集成分支并挪状态，用户
   点「完成」后前缀晋升进 main。干不下去评论原因并挪 `blocked`。
