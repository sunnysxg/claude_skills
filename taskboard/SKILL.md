...
| `in_progress` | 处理中 | 已绑定某个具体会话，那个会话正在做；父卡全程停在这里，不因为在等子卡而挪走 | 那个会话 |
...
- **目的地**（`destination`）：`production` 生产（默认）/ `dev` 开发，项目级默认可配。生产 =
  交付时系统直接把候选合并进 main、push、部署，卡进「等你确认」时成果已在线上，她点完成 = 纯
  归档；开发 = 候选由系统合入 dev 集成分支，她在常驻 dev 实例上验，点完成 = 放行、触发合并上线。
